...
const filtros: FiltroCard[] = [
  { id: "colecciones", titulo: "Colecciones", descripcion: "Todas las publicaciones disponibles.", imagen: "/ColeccionesMalvinas.jpg", badge: "Colecciones" },
  { id: "revistas", titulo: "Revistas", descripcion: "Revistas culturales y sociales.", imagen: "/RevistaAroman.png", badge: "Archivo" },
  { id: "periodicos", titulo: "Periódicos", descripcion: "Publicaciones diarias y material de hemeroteca.", imagen: "/DiarioTimes.jpeg", badge: "Informativo" },
  { id: "especiales", titulo: "Colecciones Especiales", descripcion: "Materiales únicos de valor patrimonial.", imagen: "/ColeccionesLa.png", badge: "Especial" },
];
...
function FiltroPrincipalCard({ item, activo, onClick }: { item: FiltroCard; activo: boolean; onClick: () => void }) {
  return (
    <article
      onClick={onClick}
      className={`group w-full overflow-hidden shadow-md cursor-pointer transition-all ${activo ? "border-2 border-cyan-700 bg-white ring-1 ring-cyan-600" : "border border-transparent bg-white"
        }`}
    >
      <div className="relative aspect-9/4 overflow-hidden">
        <img src={item.imagen} alt={item.titulo} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      ...
      <div className="p-4">
        <h3 className="font-serif text-xl font-black text-slate-900">{item.titulo}</h3>
        <p className="mt-2 text-sm leading-5 text-slate-600">{item.descripcion}</p>
        <span className={`mt-4 inline-flex items-center gap-2 text-sm font-semibold transition ${activo ? "text-slate-900" : "text-slate-800 group-hover:gap-3"}`}>
          {activo ? "Filtro seleccionado" : "Seleccionar filtro"} <ChevronRight className="h-4 w-4" />
        </span>
      </div>
    </article>
  );
...
function MiniCard({ item, onOpen }: { item: ItemColeccion; onOpen: () => void }) {
  return (
    <article onClick={() => item.archivoPdf && onOpen()} className="group overflow-hidden bg-slate-100 shadow-lg cursor-pointer">
      <div className="relative aspect-3/4 overflow-hidden bg-slate-200">
        <img src={item.imagen} alt={item.titulo} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" onError={(e) => (e.currentTarget.src = "/placeholder.jpg")} />
      ...
      <div className="p-4">
        <h4 className="line-clamp-2 font-serif text-lg font-black text-slate-900">{item.titulo}</h4>
        {item.descripcion && <p className="mt-2 text-sm leading-6 text-slate-600 line-clamp-2">{item.descripcion}</p>}
        {item.fecha && (
          <p className="mt-1 text-xs text-slate-500">
            {new Date(item.fecha).toLocaleDateString("es-AR", { day: "2-digit", month: "long", year: "numeric" })}
          </p>
        )}
...
export default function Colecciones() {
  ...
  const cargarItems = async (): Promise<ItemColeccion[]> => {
    try {
      const [pub1, pub2, pub3, pub4] = await Promise.all([
        fetchPublicacion_1(), fetchPublicacion_2(), fetchPublicacion_3(), fetchPublicacion_4(),
      ]);
      return [
  ...
  const listaActual = tipoActivo === "municipal" ? itemsMunicipal : itemsDigital;
  const filtroActual = tipoActivo === "municipal" ? filtroMunicipal : filtroDigital;

  const itemsFiltrados = useMemo(() =>
    filtroActual === "colecciones" ? listaActual : listaActual.filter(i => i.categoria === filtroActual),
    ...
    return (
      <PdfViewer
        ...
        collectionDate={itemData.fecha
          ...
          : itemData.numeroEdicion ? `Edición N°${itemData.numeroEdicion}` : 'Sin fecha'}
        onClose={() => setPdfActivo(null)}
        onBack={() => {
          const next = indexActual - 1;
          if (next >= 0) { setIndexActual(next); setPdfActivo(itemsFiltrados[next].archivoPdf || null); }
        }}
        onNext={() => {
          const next = indexActual + 1;
          if (next < itemsFiltrados.length) { setIndexActual(next); setPdfActivo(itemsFiltrados[next].archivoPdf || null); }
        }}
      />
    );
  }
  ...
  return (
    <section className="px-4 pb-6 pt-2 md:px-6 lg:px-8 bg-slate-50 min-h-screen">
      ...
      <div className="mt-5 mb-10 flex items-center justify-center gap-8 border-b border-slate-200">
        {[
          { id: "municipal", label: "Hemeroteca Municipal", sub: "(Archivos físicos)" },
          { id: "digital", label: "Hemeroteca Digital", sub: "(Archivos digitales)" }
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() => setTipoActivo(tab.id as TipoHemeroteca)}
            className={`pb-4 text-xl font-bold uppercase font-serif transition cursor-pointer flex flex-col items-center ${tipoActivo === tab.id ? "text-slate-900 border-b-4 border-cyan-700" : "text-slate-500 border-b-4 border-transparent hover:text-slate-800"
              }`}
          >
            <span>{tab.label}</span>
      ...
      <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4 mb-12">
        {filtros.map((f) => (
          <FiltroPrincipalCard
            ...
            onClick={() => tipoActivo === "municipal" ? setFiltroMunicipal(f.id) : setFiltroDigital(f.id)}
          />
        ))}
      </div>

      <header className="mb-8">
        <span className="text-[11px] font-bold uppercase tracking-[0.24em] text-slate-500">Resultados</span>
        <h2 className="mt-2 font-serif text-3xl font-black text-slate-900">{nombresCategoria[filtroActual]}</h2>
      </header>

      {loading ? (
        <div className="flex justify-center py-20"><div className="h-10 w-10 animate-spin rounded-full border-4 border-cyan-600 border-t-transparent" /></div>
        ...
        <div className="mt-10 py-20 text-center border-2 border-dashed border-slate-300 rounded-xl">
          <p className="font-serif text-xl font-bold text-slate-400">No se encontraron documentos en esta categoría</p>
        </div>
      )}
    </section>
  );
